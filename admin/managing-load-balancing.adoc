...
NOTE: Bien que le service StorageGRID Load Balancer soit le mécanisme d'équilibrage de charge recommandé, vous pouvez également intégrer un équilibreur de charge tiers. Pour plus d'informations, contactez votre représentant de compte NetApp ou consultez  https://docs.netapp.com/us-en/storagegrid-enable/technical-reports/load-balancer-index.html["Utilisez des équilibreurs de charge tiers avec StorageGRID"^].
